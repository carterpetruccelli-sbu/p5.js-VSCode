  ...
  demos = [
    // 1) Variables — x position updated each frame
    {
      ...
      draw() {
        ...
        for (let d of circles) {
          fill(d.x * 255 / (width), d.y * 255 / (height), 255);
          noStroke();
          circle(d.x, d.y, 40);
          d.x += 3;
          d.y += 3;
    ...
    // 5) Functions — reusable drawStar() used in a loop
    {
      ...
      draw(d) {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const y = height / 2 + sin(i) * (height / 2 - mouseY);
...

    {
      ...
      setup(d) {
        ...
        /*
    Breakdown:
      width * height → total pixels of the canvas.
      (width * height) / 10000 → ~1 item per 10,000 pixels (a density rule).
      min(200, …) → cap at 200 items max (performance guard).
      floor(...) → make it an integer (you can’t spawn a fraction of an item).
    */
        for (let i = 0; i < N; i++)
          d.P.push({
            x: random(width),
      ...
      draw(d) {
        ...
        for (let i = 0; i < d.P.length; i++) {
          ...
          p.y += p.vy;
          // wrap-around logic
          // LEFT edge: if the center goes past -radius, it’s fully off-screen → jump to just beyond RIGHT edge
          if (p.x < -p.r) p.x = width + p.r;
          // RIGHT edge: if the center goes past width+radius → jump to just beyond LEFT edge
